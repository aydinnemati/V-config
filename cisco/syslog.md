      ...
      process: |-
        ...
        function (event, emit)
            if string.find(event.log.message, "<([^ ]*)>([^ ]*):") then
                local pri, record_id  = string.match(event.log.message,"<([^ ]*)>([^ ]*):")
                local pri_facility = pri_parser_facility(pri//8)
                local pri_severity = pri_parser_severity(pri%8)
                event.log.pri_severity_detail = pri_severity_detail
                event.log.pri_severity_state = pri_severity_state
                event.log.pri_severity = pri_severity
                event.log.pri_facility_detail = pri_facility_detail
                event.log.pri_facility = pri_facility
                event.log.record_id = record_id
                local mounth, day, timee = string.match(event.log.message,"\.([^ ]*) ([^ ]*) ([^ ]*):")
                event.log.mounth = mounth
                event.log.day = day
                event.log.timee = timee
                local action = Split(string.match(event.log.message,"%%([^ ]*):"), "-")
            if action[1] == "LINK" then
                local port = string.match(event.log.message,"Interface ([^ ]*) ")
                local port_state = string.match(event.log.message,"%s(.*)")
                event.log.port = port
                event.log.port_state = port_state
            end
            emit(event)
        end
```
